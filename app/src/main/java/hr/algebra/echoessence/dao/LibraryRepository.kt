...
class LibraryRepository(context: Context) {
    ...
    fun getLibraryEntriesByUserId(userId: Int): List<Library> {
        ...
        val cursor: Cursor = db.query(
            "library", arrayOf("id", "user_id", "album", "duration", "album_cover_url", "artist_name", "artist_id"), "user_id = ?",
        ...
        while (cursor.moveToNext()) {
            ...
            libraries.add(Library(id, userId, album, duration, albumCoverUrl, artistName, artistId))
        }
        cursor.close()
        db.close()
        return libraries
